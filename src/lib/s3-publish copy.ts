...
export async function s3_copy_file(
  ...
  try {
    ...
    const getObjectCommand = new GetObjectCommand({
      ...
      Key: `${source.source_path}/${source.file_name}`,
    });
    const getObjectResponse = await client.send(getObjectCommand);

    if (!getObjectResponse.Body) {
      throw new Error("Source object does not have a valid Body.");
    }

    // Step 2: Upload to destination bucket using @aws-sdk/lib-storage Upload
    const upload = new Upload({
      ...
      params: {
        ...
        ContentType:
          getObjectResponse.ContentType ?? "application/octet-stream", // Use existing or default
      },
    });
    ...
    await upload.done();
    console.log(
      `Successfully copied file from ${source.source_bucket}/${source.source_path}/${source.file_name} to ${destination_bucket}/${Key}`,
    );
  } catch (error) {
