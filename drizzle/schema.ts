...
export const publishedArticle = pgTable(
	...
	(table) => {
		return {
			pCreatedAtIdx: index("p_created_at_idx").using(
				"btree",
...
export const draftArticle = pgTable(
	...
	(table) => {
		return {
			dCreatedAtIdx: index("d_created_at_idx").using(
				"btree",
				table.createdAt.asc().nullsLast(),
			),
			...
			draftArticlePublishedIdUnique: unique(
				"draft_article_published_id_unique",
			).on(table.publishedId),
		};
	},
);
...
export const pArticlesToAuthors = pgTable(
	...
	(table) => {
		return {
			...
			pArticlesToAuthorsPublishedIdAuthorIdPk: primaryKey({
				columns: [table.publishedId, table.authorId],
				name: "p_articles_to_authors_published_id_author_id_pk",
			}),
		};
	},
);

export const dArticlesToAuthors = pgTable(
	...
	(table) => {
		return {
			...
			dArticlesToAuthorsDraftIdAuthorIdPk: primaryKey({
				columns: [table.draftId, table.authorId],
				name: "d_articles_to_authors_draft_id_author_id_pk",
			}),
		};
	},
);
