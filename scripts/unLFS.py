...
DRY_RUN_WRITE_PATH: Final[Path] = Path("unLFS_dryRun").with_suffix(
    ".sh" if os.name != "nt" else ".ps1"
)
...
@overload
def __shellRun(
    *args, dryRun: Literal[False] = False, **kwargs
) -> subprocess.CompletedProcess: ...
@overload
def __shellRun(*args, dryRun: Literal[True] = True, **kwargs) -> str: ...
@overload
def __shellRun(
    *args, dryRun: bool = False, **kwargs
...
def __shellRun(*args, dryRun: bool = False, **kwargs):
    ...
    return subprocess.run(
        [str(x) for x in args], check=True, shell=True, capture_output=True, **kwargs
    )
...
def unLFS(
    ...
    push: bool = True,
    pushAllBranches: bool = False,
    pushRemote: str = "origin",
):
    """
    ...
    status = (
        str(__shellRun("git", "status", "--porcelain").stdout.decode("utf-8"))
        .strip()
        .splitlines()
    )
    status = [
    ...
    fileList = [
        line.split(" ").pop()
        for line in fileListOut.stdout.decode("utf-8").splitlines()
    ]
    if len(fileList) == 0:
    ...
    MERGE_BRANCH_TARGET: Final[str] = (
        __shellRun("git", "branch", "--show-current").stdout.decode("utf-8").strip()
    )
    ...
    gaLFS = [
        line
        for line in gaTextLines
        if re.search(r"filter\s*=\s*lfs", line, re.IGNORECASE)
    ]
    gaNonLFS = [
        line
        for line in gaTextLines
        if not re.search(r"filter\s*=\s*lfs", line, re.IGNORECASE)
    ]
    for lfsLine in gaLFS:
        __shellRun(
            ...
            "lfs",
            "untrack",
            re.split(r"\s", lfsLine, 0, re.IGNORECASE).pop(0),
            dryRun=dryRun,
        )
    if not dryRun:
    ...
    fileList = [
        line.split(" ").pop()
        for line in fileListRemaining.stdout.decode("utf-8").splitlines()
    ]
    if len(fileList) > 0:
        if dryRun:
    ...
    __shellRun(
        "git", "lfs", "prune", "--verify-remote", "--verbose", "--recent", dryRun=dryRun
    )
    __shellRun(
        "git stash clear && git prune --expire=now && git reflog expire --expire-unreachable=now --rewrite --all && git repack -a -d && git prune-packed && git reflog expire --all --expire=now && git gc --prune=now --aggressive",
        dryRun=dryRun,
    )
    # Remove locally
    ...
    if dryRun:
        ...
        with DRY_RUN_WRITE_PATH.open("a", encoding="utf-8") as f:
            ...
            f.write("\n")
        persistPath = DRY_RUN_WRITE_PATH.with_name(
            DRY_RUN_WRITE_PATH.with_suffix("").name
            + f"_{runID}{DRY_RUN_WRITE_PATH.suffix}"
        )
